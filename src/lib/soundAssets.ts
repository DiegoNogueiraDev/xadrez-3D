...
export const SOUND_MANIFEST: Record<SoundId, SoundAsset> = {
  ...
  ambient: { path: '/sounds/ambient.mp3', label: 'Ambient Background', format: 'mp3' },
...
export function getSoundPaths(): string[] {
  return Object.values(SOUND_MANIFEST).map((asset) => asset.path);
}
